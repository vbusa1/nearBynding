...
`r if(!CapR) {"\\begin{comment}"}`

*NOTE: this computer does not have CapR installed in the local PATH. CapR is*
*required for nearBynding to create sequence-based structure predictions.*

```{r, eval = FALSE}
runCapR(in_file = chr4and5_3UTR.fa)
```
`r if(!CapR) {"\\end{comment}"}`
`r if(CapR) {"\\begin{comment}"}`
```{r}
runCapR(in_file = chr4and5_3UTR.fa)
```
...
#### BAM file input
`r if(!bedtools) {"\\begin{comment}"}`

*NOTE: this computer does not have bedtools installed in the local PATH.* 
*bedtools is required for nearBynding to process genomic data.*
...
`r if(!stereogene) {"\\begin{comment}"}`
*NOTE: this computer does not have StereoGene installed in the local PATH.* 
*StereoGene is required for nearBynding to perform the cross-correlation*
*calculation to determine local RNA structure relative to RBP binding.*
