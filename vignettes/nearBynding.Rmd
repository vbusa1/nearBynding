...
via CapR or input their own annotations in bedGraph format. It accomodates 
protein binding information from CLIP-seq experiments as either aligned CLIP-seq 
reads or peak-called intervals. This vignette will walk you through:
...
reads or a BED file of peak intervals. BAM and BED files must be sorted and 
converted to a bedGraph file.
...
Note: the sorting function is built-in to the cleaning function for BED files.

```{r, eval = F}
# Convert BAM to BED for sake of example
# Don't actually need to use CleanBEDtoBG unless starting from a BED file
out_bed <- "bamto.bed"
...
The bedGraph files can then be mapped onto the concatenated transcriptome using 
the chain file created by `GenomeMappingToChainFile()`. This way, only the protein 
binding from transcriptomic regions of interest will be considered in the final 
...
The RNA structure information from the CapR output also needs to mapped onto the 
concatenated transcriptome. There are six different binding contexts 
...
separate bedGraph files, and then performs `liftOverToExomic()` for all the files.
...
than using CapR. If the information is in BED format, `CleanBEDtoBG()` can be 
applied to RNA structure data to convert it to a bedGraph. That bedGraph should 
then be input into `liftOverToExomicBG()` to make sure that the RNA structure data 
is mapped to the same transcriptome as the protein binding data.

## Run StereoGene

This is the process that directly answers the question, "What does RNA structure 
look like around where the protein is binding?" StereoGene is used to calculate 
the cross-correlation between RNA structure and protein binding in order to 
visualize the RNA structure landscape surrounding protein binding.

If CapR is used to determine RNA structure, `runStereogeneOnCapR()` initiates 
StereoGene for a given protein against all CapR-generated RNA structure contexts.
...
*files to your local directory if you are struggling with any of the upstream *
*steps but still wish to visualize the results.*

## Visualize Results

The cross-correlation output of StereoGene can be visualized as either a heatmap 
or a line plot. Examples of both are below.
...
```{r, fig.show='hold', echo = F, out.width = '50%'}
...
Although this specific, limited example does not provide a particularly visually 
stimulating image, larger data sets (which provide many more StereoGene windows) 
result in narrower peaks and less noise.
...
contexts are very similar, whereas larger values suggest substantial differences. 
...
We can see that the stem context is less similar to the hairpin context than the 
internal context, and this is reflected in the calculated distances.
