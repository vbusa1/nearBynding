...
the chain file created by `GenomeMappingToChainFile()`. This way, only the 
protein binding from transcriptomic regions of interest will be considered in 
...
then be input into `liftOverToExomicBG()` to make sure that the RNA structure 
data is mapped to the same transcriptome as the protein binding data.
