...
bindingContextDistanceCapR<-function(dir_stereogene_output = ".",
    ...
    if(length(protein_file_input_2) > 1){stop("Only input one background track
                                            per protein.")}
    if(!(context %in% c("all", "bulge", "hairpin", "stem",
    ...
    get_dist <- function(context){
        for(n in seq(length(protein_file))){
            assign(paste0("dist_", n),
                   read.table(paste0(dir_stereogene_output, "/", CapR_prefix,
                        "_", context, "_liftOver~", protein_file[n],'.dist'),
                        header=TRUE)) %>%
        ...
        if(!is.null(protein_file_input_2)){
            second_dist_input <- read.table(paste0(
                dir_stereogene_output_2, "/", CapR_prefix_2, "_", context,
                "_liftOver~", protein_file_input_2,'.dist'), header=TRUE) %>%
                dplyr::filter(range[1] <= x, x <= range[2])
        }
