...
GenomeMappingToChainFile <- function(genome_gtf,
  ...
  for (chr in seqnames(seqinf)) {
    for (str in c("-", "+")) {
      ...
      transcripts <- elementMetadata(gtf_hold)@listData[["transcript_id"]] %>%
        unique()
      ...
      for (t in transcripts) {
        ...
        if (length(width(ranges(gtf_t))) > 1) {
          txpt[, 2] <- c(start(ranges(gtf_t))[2:length(start(ranges(gtf_t)))] -
            (width(ranges(gtf_t))[seq(length(width(ranges(gtf_t))) - 1)] +
              start(ranges(gtf_t))[seq(length(width(ranges(gtf_t))) - 1)]) -
            1, "")
          txpt[, 3] <- 0
          txpt[nrow(txpt), 3] <- ""
        }
