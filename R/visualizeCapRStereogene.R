...
visualizeCapRStereogene<-function(dir_stereogene_output = ".",
                                  ...
                                  legend = TRUE,
                                  heatmap = FALSE){
    if(length(protein_file) < 1){stop("Requires at least one protein file prefix
                                    to calculate distance")}
    if(length(protein_file) > 20){stop("There are greater than 20 protein files
    ...
    for(n in seq(length(protein_file))){
        assign(paste0("dist_bulge_", n),
               read.table(paste0(dir_stereogene_output, "/", CapR_prefix,
                                 "_bulge_liftOver~", protein_file[n],
                                 '.dist'), header=TRUE))
        assign(paste0("dist_multibranch_", n),
               read.table(paste0(dir_stereogene_output, "/", CapR_prefix,
                                 "_multibranch_liftOver~", protein_file[n],
                                 '.dist'), header=TRUE))
        assign(paste0("dist_hairpin_", n),
               read.table(paste0(dir_stereogene_output, "/", CapR_prefix,
                                "_hairpin_liftOver~", protein_file[n],
                                '.dist'), header=TRUE))
        assign(paste0("dist_stem_", n),
               read.table(paste0(dir_stereogene_output, "/", CapR_prefix,
                                "_stem_liftOver~", protein_file[n],
                                '.dist'), header=TRUE))
        assign(paste0("dist_internal_", n),
               read.table(paste0(dir_stereogene_output, "/", CapR_prefix,
                                 "_internal_liftOver~", protein_file[n],
                                 '.dist'), header=TRUE))
        assign(paste0("dist_exterior_", n),
               read.table(paste0(dir_stereogene_output, "/", CapR_prefix,
                                "_exterior_liftOver~", protein_file[n],
                                '.dist'), header=TRUE))
    }
    ...
    dist_bulge<-as.data.frame(matrix(NA, ncol=(2*length(protein_file)) + 1,
                                     nrow = nrow(dist_bulge_1)))
    dist_stem<-as.data.frame(matrix(NA, ncol=(2*length(protein_file)) + 1,
                                    nrow = nrow(dist_bulge_1)))
    dist_multibranch<-as.data.frame(matrix(NA, ncol=(2*length(protein_file)) + 1,
                                           nrow = nrow(dist_bulge_1)))
    dist_hairpin<-as.data.frame(matrix(NA, ncol=(2*length(protein_file)) + 1,
                                       nrow = nrow(dist_bulge_1)))
    dist_exterior<-as.data.frame(matrix(NA, ncol=(2*length(protein_file)) + 1,
                                        nrow = nrow(dist_bulge_1)))
    dist_internal<-as.data.frame(matrix(NA, ncol=(2*length(protein_file)) + 1,
                                        nrow = nrow(dist_bulge_1)))
    ...
    colnames(dist_hairpin)<-c("x", paste0("Fg", seq(length(protein_file))),
                              paste0("Bkg", seq(length(protein_file))))
    dist_bulge_1$x -> dist_bulge$x -> dist_stem$x -> dist_internal$x ->
        dist_multibranch$x -> dist_exterior$x -> dist_hairpin$x
    for(n in seq(length(protein_file))){
        dist_bulge[ ,1 + n] <- eval(
            parse(text=paste0("dist_bulge_", n)))$Fg
        dist_stem[ ,1 + n] <- eval(
            parse(text=paste0("dist_stem_", n)))$Fg
        dist_hairpin[ ,1 + n] <- eval(
            parse(text=paste0("dist_hairpin_", n)))$Fg
        dist_multibranch[ ,1 + n] <- eval(
            parse(text=paste0("dist_multibranch_", n)))$Fg
        dist_exterior[ ,1 + n] <- eval(
            parse(text=paste0("dist_exterior_", n)))$Fg
        dist_internal[ ,1 + n] <- eval(
            parse(text=paste0("dist_internal_", n)))$Fg
        dist_bulge[,1+n+length(protein_file)] <- eval(
            parse(text=paste0("dist_bulge_", n)))$Bkg
        dist_stem[,1+n+length(protein_file)] <- eval(
            parse(text=paste0("dist_stem_", n)))$Bkg
        dist_hairpin[,1+n+length(protein_file)] <- eval(
            parse(text=paste0("dist_hairpin_", n)))$Bkg
        dist_multibranch[,1+n+length(protein_file)] <- eval(
            parse(text=paste0("dist_multibranch_", n)))$Bkg
        dist_exterior[,1+n+length(protein_file)] <- eval(
            parse(text=paste0("dist_exterior_", n)))$Bkg
        dist_internal[,1+n+length(protein_file)] <- eval(
            parse(text=paste0("dist_internal_", n)))$Bkg
    }
    if(!is.null(protein_file_input)){
        dist_bulge[, 2:(length(protein_file)+1)]<-dist_bulge[,
                   2:(length(protein_file)+1)]-dist_bulge_input$Fg
        dist_bulge[, (length(protein_file)+2):ncol(dist_bulge)]<-
            dist_bulge[, (length(protein_file)+2):ncol(dist_bulge)]-
            dist_bulge_input$Bkg
        dist_stem[, 2:(length(protein_file)+1)]<-dist_stem[,
                    2:(length(protein_file)+1)]-dist_stem_input$Fg
        dist_stem[, (length(protein_file)+2):ncol(dist_stem)]<-
            dist_stem[,(length(protein_file)+2):ncol(dist_stem)]-
            dist_stem_input$Bkg
        dist_hairpin[, 2:(length(protein_file)+1)]<-dist_hairpin[,
                    2:(length(protein_file)+1)]-dist_hairpin_input$Fg
        dist_hairpin[, (length(protein_file)+2):ncol(dist_hairpin)]<-
            dist_hairpin[,(length(protein_file)+2):ncol(dist_hairpin)]-
            dist_hairpin_input$Bkg
        dist_multibranch[, 2:(length(protein_file)+1)]<-dist_multibranch[,
                    2:(length(protein_file)+1)]-dist_multibranch_input$Fg
        dist_multibranch[, (length(protein_file)+2):ncol(dist_multibranch)]<-
            dist_multibranch[, (length(protein_file)+2):ncol(dist_multibranch)]-
            dist_multibranch_input$Bkg
        dist_internal[, 2:(length(protein_file)+1)]<-dist_internal[,
                    2:(length(protein_file)+1)]-dist_internal_input$Fg
        dist_internal[, (length(protein_file)+2):ncol(dist_internal)]<-
            dist_internal[, (length(protein_file)+2):ncol(dist_internal)]-
            dist_internal_input$Bkg
        dist_exterior[, 2:(length(protein_file)+1)]<-dist_exterior[,
                    2:(length(protein_file)+1)]-dist_exterior_input$Fg
    ...
    if(length(protein_file) == 1){
        ...
        dist_bulge$Fg<-rowMeans(dist_bulge[,
        ...
        dist_hairpin$Bkg_se<-rowSds(as.matrix(dist_hairpin[,
                    (length(protein_file)+2):((length(protein_file)*2)+1)]))/
            sqrt(length(protein_file))
        dist_stem$Fg<-rowMeans(dist_stem[,
                                2:(length(protein_file)+1)])
    ...
    if(heatmap == F){ # create line plot
        ...
        if(is.null(y_lim)){
            max_y<-max(c(max(dist_exterior$Fg),
                         max(dist_internal$Fg),
                         max(dist_stem$Fg),
                         max(dist_hairpin$Fg),
                         max(dist_multibranch$Fg),
                         max(dist_bulge$Fg)))+.1
            min_y<-min(c(min(dist_exterior$Fg),
                         min(dist_internal$Fg),
                         min(dist_stem$Fg),
                         min(dist_hairpin$Fg),
                         min(dist_multibranch$Fg),
                         min(dist_bulge$Fg)))-.1
            y_lim<-c(min_y, max_y)
        }

        plot(dist_bulge$x, dist_bulge$Bkg, type = 'l',col = 'black',
             xlim = x_lim, ylim = y_lim,
             main = NULL, xlab = 'Distance',
             ylab ='Density x 100',cex.axis = 0.8,  cex.lab = 1,
             cex.main = 1.2, lwd=2)
        ...
        arrows(dist_bulge$x, dist_bulge$Bkg-dist_bulge$Bkg_se,
               dist_bulge$x, dist_bulge$Bkg+dist_bulge$Bkg_se, length=0,
               angle=90, code=3,
               col = rgb(blue = 0, red = 0, green = 0, alpha = .5))
        arrows(dist_multibranch$x, dist_multibranch$Bkg-dist_multibranch$Bkg_se,
               dist_multibranch$x, dist_multibranch$Bkg+dist_multibranch$Bkg_se,
        ...
        arrows(dist_hairpin$x, dist_hairpin$Bkg-dist_hairpin$Bkg_se,
               ...
               length=0, angle=90, code=3,
               col = rgb(blue = 0, red = 0, green = 0, alpha = .5))
        arrows(dist_internal$x, dist_internal$Bkg-dist_internal$Bkg_se,
               dist_internal$x, dist_internal$Bkg+dist_internal$Bkg_se,
               length=0, angle=90, code=3,
               col = rgb(blue = 0, red = 0, green = 0, alpha = .5))
        arrows(dist_exterior$x, dist_exterior$Bkg-dist_exterior$Bkg_se,
               dist_exterior$x, dist_exterior$Bkg+dist_exterior$Bkg_se,
               length=0, angle=90, code=3,
               col = rgb(blue = 0, red = 0, green = 0, alpha = .5))
        ...
        arrows(dist_multibranch$x, dist_multibranch$Fg-dist_multibranch$Fg_se,
               dist_multibranch$x, dist_multibranch$Fg+dist_multibranch$Fg_se,
               length=0, angle=90, code=3,
               col = rgb(blue = 0, red = 1, green = 0, alpha = .5))
        arrows(dist_stem$x, dist_stem$Fg-dist_stem$Fg_se,
               dist_stem$x, dist_stem$Fg+dist_stem$Fg_se,
               length=0, angle=90, code=3,
               col = rgb(blue = 0, red = 0, green = 1, alpha = .5))
        arrows(dist_hairpin$x, dist_hairpin$Fg-dist_hairpin$Fg_se,
               dist_hairpin$x, dist_hairpin$Fg+dist_hairpin$Fg_se,
               length=0, angle=90, code=3,
               col = rgb(blue = 1, red = 1, green = 0, alpha = .5))
        arrows(dist_internal$x, dist_internal$Fg-dist_internal$Fg_se,
               dist_internal$x, dist_internal$Fg+dist_internal$Fg_se,
               length=0, angle=90, code=3,
               col = rgb(blue = 0, red = 1, green = 1, alpha = .5))
        arrows(dist_exterior$x, dist_exterior$Fg-dist_exterior$Fg_se,
