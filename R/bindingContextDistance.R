...
#' @param CapR CapR was used to generate the RNA structure information.
#' Default TRUE.
...
#' protein_file_2, if applicable.Equivalent to output_prefix from
#' runStereogeneOnCapR. Default ""
...
bindingContextDistance<-function(dir_stereogene_output = ".",
    ...
    if(length(protein_file_input) > 1){stop("Only input one background track
                                          per protein.")}
    if(length(protein_file_2) < 1){stop("Requires at least one protein file
                                        prefix to calculate distance")}
    if(length(protein_file_2) > 20){stop("There are greater than 20 protein
                                         files input. This is likely in error")}
    if(length(protein_file_input_2) > 1){stop("Only input one background track
                                            per protein.")}
    if(CapR == TRUE & !(context %in% c("all", "bulge", "hairpin", "stem",
                                       "exterior", "multibranch", "internal"))){
    ...
    get_dist<-function(context){
        ...
        for(n in seq(length(protein_file_2))){
            assign(paste0("second_dist_", n),
                   read.table(paste0(dir_stereogene_output_2, "/",
                                 prefix_2, context, "_liftOver~",
                                 protein_file_2[n],'.dist'), header=TRUE)) %>%
                                 dplyr::filter(range[1] <= x, x <= range[2])
        }
        if(!is.null(protein_file_input_2)){
            second_dist_input <- read.table(paste0(
                dir_stereogene_output_2, "/", prefix_2, context, "_liftOver~",
        ...
        for(n in seq(length(protein_file_2))){
            second_dist[,1 + n] <- eval(
                parse(text=paste0("second_dist_", n)))$Fg
            second_dist[,1 + n + length(protein_file_2)] <- eval(
                parse(text=paste0("second_dist_", n)))$Bkg
        }
        if(!is.null(protein_file_input_2)){
    ...
    if(CapR == TRUE & context == "all"){
        distance<-sum(
            ...
            get_dist("exterior"),
            get_dist("stem"),
            get_dist("multibranch")
        )}
    else{distance <- get_dist(context)}
