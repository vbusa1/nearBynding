...
visualizeStereogene<-function(dir_stereogene_output = ".",
    ...
    if(heatmap == FALSE){ # create line plot
        ...
        plot(dist$x, dist$Bkg, type = 'l',col = 'black',
             ...
             cex.main = 1.2, lwd=2)
        arrows(dist$x, dist$Bkg-dist$Bkg_se,
               dist$x, dist$Bkg+dist$Bkg_se, length=0,
               angle=90, code=3,
               col = rgb(blue = 0, red = 0, green = 0, alpha = .5))
        abline(v=0, col="grey", lty=2)
        lines(dist$x, dist$Fg , col='blue',lwd=2)
        arrows(dist$x, dist$Fg-dist$Fg_se,
        ...
        if(is.null(y_lim)){
            max_y<-max(c(max(dist$Bkg),
                         max(dist$Fg)))+.1
            min_y<-min(c(min(dist$Bkg),
                         min(dist$Fg)))-.1
